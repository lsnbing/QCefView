...
class QCefQMLView;
class QCefQMLViewPrivate;

#define QMLIsValidBrowser(browser)                                                                                        \
  (pCefViewPrivate_ && pCefViewPrivate_->pCefBrowser_ &&                                                     \
   browser->IsSame(pCefViewPrivate_->pCefBrowser_))

class CCefQMLClientDelegate
  : public CefViewBrowserClientDelegateInterface
  , public std::enable_shared_from_this<CCefQMLClientDelegate>
{
public:
  typedef std::shared_ptr<CCefQMLClientDelegate> RefPtr;
  typedef std::weak_ptr<CCefQMLClientDelegate> WeakPtr;

private:
  QCefQMLViewPrivate* pCefViewPrivate_;

  QMap<qint32, QCefDownloadItemPointer> downloadItemMap_;

public:
  CCefQMLClientDelegate(QCefQMLViewPrivate* p);

  ~CCefQMLClientDelegate();
  ...
  virtual void onContextMenuDismissed(CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame) override { return;
  }
  ...
  virtual bool tooltipMessage(CefRefPtr<CefBrowser>& browser, const std::string& text) override { return true; }
  ...
  virtual bool setFocus(CefRefPtr<CefBrowser>& browser) override { return true; }
